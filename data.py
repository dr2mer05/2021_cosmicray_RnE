...
load_file_name_e = ""
load_file_type = "xlsx"
load_file_path_mu = "{}/{}.{}".format(load_dir_path,
                                      load_file_name_mu, load_file_type)
load_file_path_e = "{}/{}.{}".format(load_dir_path,
                                     load_file_name_e, load_file_type)
...
for i in range(1, 11):  # 1~10GeV
    ...
    plt.hist(listRe_mu, bins=range(0, 1000, 1),
             density=True, alpha=1, histtype='stepfilled')
    plt.axis('off')
    ...
    plt.hist(listRe_mu, bins=range(0, 1000, 1),
             density=True, alpha=1, histtype='stepfilled')
    plt.axis('off')
    load_file_path_mu = "{}/{}%d.{}".format(
        save_dir_path_mu, save_file_name_mu, i, save_file_type)
    plt.savefig(load_file_path_mu)
    plt.close()

    plt.hist(listRe_e, bins=range(0, 1000, 1),
             density=True, alpha=1, histtype='stepfilled')
    plt.axis('off')
    load_file_path_e = "{}/{}%d.{}".format(save_dir_path_e,
                                           save_file_name_e, i, save_file_type)
